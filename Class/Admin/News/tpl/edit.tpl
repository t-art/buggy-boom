<h3 class="acenter"><?=$data['id'] ? 'Редактирование' : 'Добавление'?> новости</h3>
<form id="frmEdit" action="./index.php?request=<?=$this->_commonObj->_objectName?>/save&id=<?=$data['id']?>" method="post">
  ...
  <table class="edit" cellspacing="0" cellpadding="0" align="center">
    <tr class="even">
      <th>Дата:</th>
      <td><input type="text" id="date" name="date" value="<?=$data['id'] ? htmlspecialchars($data['datef'], ENT_QUOTES) : date('d.m.Y')?>"></td>
    </tr>
    <tr class="odd">
      <th>Название:</th>
