...
<table align="center" cellspacing="0" cellpadding="0" class="list" width="100%">
  ...
  foreach ($data['items'] as $item) {
    ...
    <tr class="<?=($i%2 == 0) ? 'odd' : 'even'?> acenter">
      <td><img src="/img/<?=$this->_commonObj->_objectName?>/<?=$item['id']?>.jpg" width="218"><input type="hidden" name="item[<?=$item['id']?>]" value="<?=$item['id']?>"></td>
      <td class="aleft"><a href="<?=$item['link']?>" target="_blank"><?=$item['link']?></a></td>
