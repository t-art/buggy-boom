<h3 class="acenter">Текстовые страницы</h3>
...
<table align="center" cellspacing="0" cellpadding="0" class="list" width="100%">
  <tr>
    ...
    <th width="100">Отображать в шапке</th>
  ...
  foreach ($data['items'] as $item) {
    ...
    <tr class="<?=($i%2 == 0) ? 'odd' : 'even'?> acenter">
      ...
      <?php
        ...
        ?>
        <td class="aleft"><a href="./index.php?request=<?=$this->_commonObj->_objectName?>/list&id=<?=$item['id']?>"><?=$item['name']?></a><input type="hidden" name="item[<?=$item['id']?>]" value="<?=$item['id']?>"></td>
      <?php
      }
      ?>
      <td><input type="checkbox" name="in_header[<?=$item['id']?>]" value="1" <?=$item['in_header'] ? 'checked' : ''?>></td>
